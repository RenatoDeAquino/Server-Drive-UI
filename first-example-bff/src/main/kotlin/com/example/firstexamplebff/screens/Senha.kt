...
class Senha {
    ...
    fun createSenha(): Screen = Screen(child = Container(children = listOf(
            Text(text = "Digite sua senha").applyStyle(style=style),
            Button(text = "Digitei minha senha", onPress = listOf(Navigate.PushStack(route = Route.Remote("/homepage")))).applyStyle(style=buton)
            )
        )
    )
}